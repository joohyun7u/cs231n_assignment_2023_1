...
def svm_loss(x, y):
    ...
    num_train = x.shape[0]
    margin = np.maximum(0,x - x[np.arange(x.shape[0]),y].reshape((-1,1)) + 1)
    margin[np.arange(margin.shape[0]),y] = 0
    # loss = np.mean(np.sum(margin,axis=1)) + reg*np.sum(W*W)
    loss = np.sum(margin)/num_train
    dx = np.zeros(x.shape)
    dx[margin>0]=1
    dx[np.arange(dx.shape[0]),y] = -np.sum(margin>0,axis=1)
    dx /= num_train
    pass
